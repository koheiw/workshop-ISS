...
dat <- cbind(dat_lda, dat_lss[c("lss")], dat_dic[c("us", "cn", "ru", "kr", "kp")])
...
```{r}
reg3 <- lm(lss ~ us + cn + ru + kp + kr, dat_sec)
summary(reg3)
```

### Countries and post-1991

```{r}
reg4 <- lm(lss ~ us * p91 + cn * p91 + ru * p91 + kp * p91 + kr * p91, dat_sec)
summary(reg4)
```
